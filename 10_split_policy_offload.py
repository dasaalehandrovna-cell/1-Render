...
def r29_assert_r28_fast_ui_contract() -> bool:
    """R51 semantic guard: canonical renderer must detach Telegram I/O."""
    fn=globals().get('fast_ui_edit_message_text'); canonical=globals().get('_canon_fast_ui_edit_message_text__001')
    if not callable(fn): raise RuntimeError('R51 FAST UI CONTRACT: renderer missing')
    if not callable(canonical) or fn is not canonical: raise RuntimeError('R51 FAST UI CONTRACT: canonical owner mismatch')
    try: source=inspect.getsource(canonical)
    except Exception as exc: raise RuntimeError('R51 FAST UI CONTRACT: cannot inspect renderer: '+str(exc))
    if 'WINDOW_RENDER_TASK_POOL.submit_latest' not in source or '_r22_execute_window_render' not in source:
        raise RuntimeError('R51 FAST UI CONTRACT: latest render actor missing')
    if '_perform_fast_ui_edit(payload)' in source:
        raise RuntimeError('R51 FAST UI CONTRACT: callback renderer performs Telegram I/O directly')
    return True
...
try:
    ...
    if 'WINDOW_RENDER_TASK_POOL.submit_latest' not in _src or '_perform_fast_ui_edit(payload)' in _src:
        raise RuntimeError('R51 HOTPATH GUARD: detached latest render contract changed')
except OSError:
    pass
