...
def run_manual_mega_restore(chat_id: int):
    ...
    success = False
    try:
        send_and_auto_delete(chat_id, '☁️ 12.27: читаю MEGA current_manifest → immutable generation → current_tail…', 30)
...
def run_manual_redis_restore(chat_id: int):
    ...
    chat_id=int(chat_id)
    try:
        send_and_auto_delete(chat_id,'⛔ 12.27: восстановление из Redis отключено. Redis используется только как cache/locks/ускоритель. Каноническое восстановление выполняется только из MEGA.',180)
