# v262
"""Пер-R29 UX/policy layer.

R29 is deliberately loaded LAST.  It changes only local policy/UI composition and
keeps the R28 hot path intact: ordinary callback -> handler -> direct Telegram edit.
No render queue, remote HTTP or heavy snapshot is inserted before a visual response.
"""

R29_RELEASE_NAME = 'Пер-R29'
R29_RELEASE_STAGE = 'directive-google-info-input-sources'
...
try:
    bot_journal('r29_policy_ui_ready', int(OWNER_ID or 0),
                'r28_direct_render_protected=1; directive_business_lock=1; google_single_window=1; compact_info=1; input_sources=forwarded+other_bots')
except Exception:
    pass
# v262
