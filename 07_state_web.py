...
def _r22_accept_callback_fast(payload: dict, update_id, update_chat_id, update_key):
    ...
    if claim_state == 'new':
        try:
            ...
            # R51: every visible-window callback gets an epoch.  Safe navigation may
            # supersede the whole handler; ordered business callbacks still execute
            # their state mutation, but an older callback is forbidden from painting
            # over a newer user action after its mutation finishes.
    ...
    if claim_state == 'new':
        ...
        # R51: never accept minutes of ordered business work into RAM.  Critical
        # mutations are not dropped: on saturation we return 503 before HTTP ACK so
        # Telegram remains the durable retry queue.  This is fundamentally different
        # from visual latest-wins renders, which are safe to replace.
        if selected_pool is globals().get('FAST_UI_TASK_POOL'):
            try:
                _ks = selected_pool.key_status(selected_key)
                _ps = selected_pool.stats()
                _per_key_max = max(2, min(16, int(os.getenv('R51_BUSINESS_MAX_QUEUED_PER_KEY','4') or '6')))
                _total_max = max(12, min(100, int(os.getenv('R51_BUSINESS_MAX_PENDING','20') or '48')))
                if int(_ks.get('queued') or 0) >= _per_key_max or int(_ps.get('pending') or 0) >= _total_max:
                    UPDATE_DISPATCHER.release_failed_enqueue(update_id, 'fast_ui_business_backpressure')
                    try: log_error(f'R51 BUSINESS BACKPRESSURE update={update_id} key={selected_key} queued={_ks.get("queued")} pending={_ps.get("pending")}')
                    except Exception: pass
                    return ('BUSY', 503)
            except Exception:
                pass
